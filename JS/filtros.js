...
function showDropdown(columnIndex) {
  ...
  const filterText = input ? input.value.toLowerCase() : ""; // Texto digitado no filtro, ou vazio se input for nulo
  ...
  for (let i = 0; i < rows.length; i++) {
    const cell = rows[i]?.getElementsByTagName("td")[columnIndex - 1]; // Ignora linhas nulas ou sem a célula desejada
    if (cell) {
      const value = cell.innerText?.trim(); // Ignora valores nulos ou indefinidos
  ...
  uniqueValues.forEach(value => {
    if (value) { // Ignora valores nulos ou vazios
...
function applyFilters() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let columnIndex = 0; columnIndex < filters.length; columnIndex++) {
      ...
      // Verifica se o filtro atual existe e não é nulo/undefined
      if (cell && filters[columnIndex]) {
        const value = cell.innerText.trim();

        // Aplica o filtro se ele tiver valores definidos
...
/* conta as series linha */
 function updateVisibleRowCount() {
  const table = document.getElementById("sortableTable");
  const rows = table.getElementsByTagName("tbody")[0].getElementsByTagName("tr");

  // Conta apenas as linhas que estão visíveis
  ...
  rowCountElement.textContent = `Qtde itens: ${visibleRowCount}`;
}


// Chama a função para garantir que o número seja atualizado inicialmente
document.addEventListener("DOMContentLoaded", updateVisibleRowCount);